SOURCES=$(shell python3.9  scripts/read-config.py --sources )
FAMILY=$(shell python3.9  scripts/read-config.py --family )
...
dev: venv .init.stamp sources/config-dev.yaml $(SOURCES)
	. venv/bin/activate; rm -rf fonts/; gftools builder sources/config-dev.yaml; python3.9 .9 scripts/addVersionToNames.py ./fonts/variable;

venv: venv/touchfile

build.stamp: venv .init.stamp sources/config.yaml $(SOURCES)
	. venv/bin/activate; rm -rf fonts/; gftools builder sources/config.yaml && touch build.stamp; rm -rf instance_ufo/; python3.9  scripts/fixTTHinting.py; python3.9  scripts/makeWebfonts.py

.init.stamp: venv
	. venv/bin/activate; python3.9  scripts/first-run.py
